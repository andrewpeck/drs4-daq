...
void CreatePlots::Loop()
{
  ...
  // corrected: 
  TH2D* scintVsTDCtime_corr = new TH2D(name="scintVsTDCtime_corr",title="scintillator hit time vs average corrected TDC time",nXBins=100,xLow=28,xHigh=37,nYBins=100,yLow=3.25,yHigh=3.75);
...
TH1D* leftTDCcorrHist = new TH1D("leftTDCcorrHist","corrected threshold timing, left",100,0,40);
  ...
  TH2D* TDCdiff_constVsADC = new TH2D(name="TDCdiff_constVsADC",title="constant timing difference vs avg total charge",nXBins=100,xLow=0,xHigh=100e-12,nYBins=100,yLow=-7,yHigh=7);
  TH2D* TDCdiff_fracVsADC = new TH2D(name="TDCdiff_fracVsADC",title="fractional timing difference vs avg total charge",nXBins=100,xLow=0,xHigh=100e-12,nYBins=100,yLow=-7,yHigh=7);
  TH2D* TDCdiff_corrVsADC = new TH2D(name="TDCdiff_corrVsADC",title="corrected timing difference vs avg total charge",nXBins=100,xLow=0,xHigh=100e-12,nYBins=100,yLow=-7,yHigh=7);

  TH1D* ADChist[2];
  ADChist[0] = new TH1D("ADChist0","histogram of ADC values",100,0,100e-12);
  ADChist[1] = new TH1D("ADChist1","histogram of ADC values",100,0,100e-12);
 
  TH1D* ADCrootHist[2];
  ADCrootHist[0] = new TH1D("ADCrootHist0","histogram of ADC values",50,0,10);
  ADCrootHist[1] = new TH1D("ADCrootHist1","histogram of ADC values",50,0,10);

  // TDC histograms sliced for ranges of ADC
  const unsigned int TDCslices = 50;
  std::string TDChistName = "TDChist_slice";
  TH1D* TDChists[TDCslices];
  for(unsigned int i=0; i<TDCslices; i++)
    {
      std::stringstream ss;
      ss << i;
      std::string Tname = TDChistName + ss.str();
      TDChists[i] = new TH1D(Tname.c_str(),Tname.c_str(),50,29,34);
    }
  TGraph* TDCresADCslices = new TGraph();
  TDCresADCslices->SetTitle("TDC resolution in slices of ADC values");
  TDCresADCslices->GetXaxis()->SetTitle("charge (C)");
  TDCresADCslices->GetXaxis()->CenterTitle();
  TDCresADCslices->GetYaxis()->SetTitle("timing resolution (ns)");
  TDCresADCslices->GetYaxis()->CenterTitle();
  unsigned int ch = 0; //chamber
  ////////// End Setup for Histograms ////////////////////////////
  ...
  for (Long64_t jentry=0; jentry<nentries; jentry++) 
	...
	for(unsigned int n=0; n<2; n++)
	  pmtHitsVSecThetaProf[n]->Fill(1/cos(gunTheta),pmtHitCount[n]);}	  
      // apply cut for angle
      if(gunTheta > .949)
	continue;
      ...
      for (unsigned int n=0; n<2; n++)
	{
	  ...
	  if(hitPointX >= -25 && hitPointX <= 25)
	    {
	      ADChist[n]->Fill(totalCharge[n]*cos(gunTheta));
	      ADCrootHist[n]->Fill(sqrt(totalCharge[n]*cos(gunTheta)/.9e-11)); // puts in units of charge
	      if (CTHtime[n] != -1) {
      ...
      double avgCharge = (totalCharge[0]+totalCharge[1])/2;
	...
	rightTDCfracHist->Fill(FTHtime[1]);
      ...
      TDCdiff_constVsADC->Fill(avgCharge,timeDiff_const);
      TDCdiff_fracVsADC->Fill(avgCharge,timeDiff_frac);

    } /////// end loop through tree first time
  
  std::cout << "chargemin" << chargeMin[0] << " " << chargeHist[0]->GetMinimum()<< std::endl;
  std::cout << "chargemin" << chargeMin[1] << " " << chargeHist[1]->GetMinimum()<< std::endl;
  std::cout << "chargemax" << chargeMax[0] << " " << chargeHist[0]->GetMaximum()<< std::endl;
  std::cout << "chargemax" << chargeMax[1] << " " << chargeHist[1]->GetMaximum()<< std::endl;
  ...
  ///////// loop through tree second time to fill corrected values ///////
  for (Long64_t jentry=0; jentry<nentries; jentry++) 
    {
      Long64_t ientry = LoadTree(jentry);
      if (ientry < 0) break;
      ...
      for(unsigned int n=0; n<2; n++) // loop through pmts
	{
	  ...
	  if(hitPointX >= -25 && hitPointX <= 25 && CTHtime[n]!=-1)
	    ...
	    } // fill the third histogram with points from both
	} // end loop through pmts
	...
	rightTDCcorrHist->Fill(correctedTiming[1]);
      ...
      double avgCharge = (totalCharge[0]+totalCharge[1])/2;
      TDCdiff_corrVsADC->Fill(avgCharge,timeDiff_corr);
      
      // fill slices of TDC histogram
      if(totalCharge[ch] < (chargeMin[ch]+chargeMax[ch])/2) {
	double findSlice = (totalCharge[ch]-chargeMin[ch])/(chargeMax[ch]-chargeMin[ch])*2*TDCslices;
	int slice = trunc(findSlice);
	if(slice == TDCslices)
	  slice--;
	TDChists[slice]->Fill(CTHtime[ch]);
      }
    } // end loop through tree second time for corrected values
  ...
  resCanvas->Print("plots/deltaTvsX.png","png");
  ...
  peakChargeCanvas->Print("plots/PeakVoltageTotalCharge.png","png");
  timeCanvas->Print("plots/ThresholdTiming.png","png");
  canvasTDCADC->Print("plots/TDCvADC.png","png");
  ...
  canvas->Print("plots/hitsOverST.png","png");
  ...
  canvas->Print("plots/pmtHitsVSecTheta.png","png");
  ...
  for(unsigned int n=0; n<2; n++) {
    canvas->cd(n+1);
    pmtHitsVx[n]->Draw(); }
  canvas->Print("plots/pmtHitsVx.png","png");
  ...
  for(unsigned int n=0; n<2; n++) {
    canvas->cd(n+1);
    pmtHitsVy[n]->Draw(); }
  canvas->Print("plots/pmtHitsVy.png","png");

    //draw ADC histogram
  canvas->SetTitle("ADChist");
  for(unsigned int n=0; n<2; n++) {
    canvas->cd(n+1);
    ADChist[n]->Draw(); }
  canvas->Print("plots/ADChist.png","png");
  
  // in charge units
  canvas->SetTitle("ADCrootHist");
  for(unsigned int n=0; n<2; n++) {
    canvas->cd(n+1);
    ADCrootHist[n]->Draw(); }
  canvas->Print("plots/ADCrootHist.png","png");
  ...
  slewCanvas->Print("plots/TDCvADC_fit.png","png");
  ...
  scintTimeCanvas->Print("plots/scintVsTDCtime.png","png");
  ...
  deltaTCanvas->cd(3);
  deltaTcorrHist->Draw();
  deltaTCanvas->Print("plots/deltaTHists.png","png");
  ...
  avgPECanvas->cd(1);
  avgPEHist->Draw();
  avgPECanvas->Print("plots/avgPEhist.png","png");
  ...
  primaryEnergyCanvas->Print("plots/primaryEnergyhist.png","png");

  TCanvas* TDCdiffVsADCCanvas = new TCanvas("TDCdiffVsADCCanvas","primary energy canvas", 10, 10, 700, 1500);
  TDCdiffVsADCCanvas->Divide(1,3);
  TDCdiffVsADCCanvas->cd(1);
  TDCdiff_constVsADC->Draw("COLZ");
  TDCdiffVsADCCanvas->cd(2);
  TDCdiff_fracVsADC->Draw("COLZ");
  TDCdiffVsADCCanvas->cd(3);
  TDCdiff_corrVsADC->Draw("COLZ");
  TDCdiffVsADCCanvas->Print("plots/TDCdiffVsADC.png","png");

  TCanvas* TDCsliceCanvas = new TCanvas("TDCsliceCanvas","TDC hists sliced in ADC value", 10, 10, 700, 1500);
  TDCsliceCanvas->Divide(4,5);
  TF1* gausFitTDC = new TF1("gausFitTDC","gaus",0,50);
  std::ofstream TDCsliceFile;
  TDCsliceFile.open("TDCslicePoints.dat");
  double TDCslicePoints[TDCslices][4];
  for(unsigned int i=0; i<TDCslices; i++)
    {
      TDCsliceCanvas->cd(i+1);

      TDChists[i]->Fit(gausFitTDC);
      double chargeMidPoint = chargeMin[ch]+(i+.5)/TDCslices*(chargeMax[ch]-chargeMin[ch]);
      TDCresADCslices->SetPoint(i,chargeMidPoint,gausFitTDC->GetParameter(2));
      TDCslicePoints[i][0] = chargeMidPoint;
      TDCslicePoints[i][1] = gausFitTDC->GetParameter(2);
      TDCslicePoints[i][2] = gausFitTDC->GetParError(2);
      TDCslicePoints[i][3] = TDChists[i]->GetEntries();
      //TDCsliceFile << "{" << chargeMidPoint << ", " << gausFitTDC->GetParameter(2) << ", " << gausFitTDC->GetParError(2) << ", " << TDChists[i]->GetEntries() << "}" << std::endl;
    }
  if(TDCsliceFile.is_open()) {
    for(unsigned int j=0; j<4; j++) {
      TDCsliceFile << "{";
      for(unsigned int i=0; i<TDCslices; i++) {
	TDCsliceFile << TDCslicePoints[i][j];
	if(i < TDCslices-1)
	  TDCsliceFile << ", ";
      }
      TDCsliceFile << "}" << std::endl;
    }
  }
	
  
  TDCsliceFile.close();
  TDCsliceCanvas->Print("plots/TDCslices.pdf","pdf");
  TCanvas* TDCresADCsliceCanvas = new TCanvas("TDCresADCslice","time resolution vs total charge", 10, 10, 800, 700);
  TDCresADCsliceCanvas->cd(1);
  TDCresADCslices->Draw("AP*");
  TDCresADCsliceCanvas->Print("plots/TDCresADCslice.png","png");
  ...
  TDCdiffVsADCCanvas->Close();
  TDCsliceCanvas->Close();
  ////// end draw plots ////////////
