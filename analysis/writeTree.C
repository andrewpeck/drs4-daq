...
int	processNum=0;		//number in which this was processed (basically a better event number)
...
//scintillator info
vector<vector<double>> scintEdep;	//energy deposits in the scint slabs
std::vector<double> scintTimes;	//times for hits in each scint slab
vector<double> 	scintEdepTotal;	//total energy deposits in the scint slabs
...
double threshold = 12.0; //constant threshold (mV) average 60mV so this is 10%
...
{  
  ...
  outFileName = "testTree.root";
  inFileName = "test.dat";

  // Create output file
  //TFile tree_file("testTree.root","RECREATE");
  ...
  tree1->Branch("secTheta", &secTheta, "secTheta/D");
  ...
  tree1->Branch("scintEdepTotal", &scintEdepTotal);
  ...
  while(!fp.eof())
      ...
      if(line0==77777)
	{
	  ...
	  // was event number, process number more useful
	  std::cout << "process number set to " << processNum << std::endl;
	  processNum = eventCounter;
	  eventCounter++;
	}
      //read event data
      else if(line0==88888)
	{
	  primaryEnergy = lineData[1];
	  gunTheta = lineData[2];
	  secTheta = 1/cos(gunTheta);
	  gunPhi = lineData[3];
	  hitPointX = lineData[4];
	  hitPointY = lineData[5];
	}
      else if (line0==55555)
	{
	  enteredLG[0] = lineData[1];
	  enteredLG[1] = lineData[2];
	}
	  
      //if this is scintillator energy deposit type of event, process it
      else if(line0%100==10)
	{  
	  scintEdep.push_back(lineData[1]);
	  int i = (line0-line0%100)/100;
	  if(i > scintEdepTotal.size())
	    scintEdepTotal.push_back(0);
	  scintEdepTotal.at(i) += lineData[1];
	  scintTimes.push_back(lineData[2]);
	}
      
      //process pmt hits
      else if((line0%100-line0%10)==50)
